...
```{r S3-temperature-0, fig.height=3, fig.width=5}
ggplot(S3_merged) +
  geom_histogram(aes(x = Temp, fill = label)) +
  labs(title = "S3 Chest Measured Body Temperature") 
# Note there are abnormally low body tempratures (15.42278 degrees) detected; 
# 100 observations in the downsampled dataset; all under stressed condition
```
...
S3_merged <- S3_merged %>%
 ...
 # filter out ~15 degrees body temperature
...
p15 = ggplot(S2_merged) +
  geom_histogram(aes(x = HR_EMP4, fill = label)) +
  labs(title = "S2 Wrist Heart Rate")

p16 = ggplot(S2_merged) +
