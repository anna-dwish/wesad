...
In the Appendix, you can see a full data dictionary in Table 1 that defines the metrics, their sampling rates, and any extracted features. Finally, it is important to note that for Subject 3, there appears to have been a brief malfunction (slightly less than 1 second) that led to impossible measurements, such as temperatures of -273$^\circ$ C. We chose to treat these ranges as missing values and also used linear interpolation to replace these faulty measurements.
...
Finally, we performed feature extraction from these raw sensor values by calculating general transformations, such as mean and standard deviation, over a 2 second window and 0.25 second window shift. Please refer to Table 1 in the Appendix for more details on other transformations we explored in generating our models. This decision was motivated by a similar study that used the *Empatica E4* sensor (FIND REF) and was also tested for sensitivity using a 3 second window and 0.5 second window shift and a 2 second window with a 0.5 second window shift. The results of these window and shift pairings can be found in the Appendix in Table ** (REMOVE IF FALSE).

For this paper, we have decided to generate several logistic regression models to better understand and interpret the relationship between these metrics. We began by conducting principal component analysis, given the large number of predictors, along with exploring histograms and scatterplots of our metrics. From this we started with a model with the following predictors: .... and ran 10-fold cross-validation on our dataset to estimate the test error. Note that it was necessary to include the subject id in order for us to be able to treat our observations as independent.
...
collapse_rows_dt <- data.frame(Metric = c(rep("ECG (mV)", 3), rep("EDA ($\\mu$s)", 2), rep("EMG (mV)", 2), rep("Temp (C$^\\circ$)", 2),
                               ...
                               Description = linebreak(c(rep("Electrocardiography: 700 Hz", 3), 
                                                         ...
                                                         rep("Electromyography: 700 Hz", 2),
                                                         rep("Body Temperature: 700 Hz", 2), 
                                                         rep("Respiration: 700 Hz", 3), 
                                                         rep("Blood Volume Pulse: 64 Hz", 3), 
...
9. Gjoreski, M., Luštrek, M., Gams, M., &amp; Gjoreski, H. (2017, August 10). Monitoring stress with a wrist device using context. Retrieved September 27, 2020, from https://www.sciencedirect.com/science/article/pii/S1532046417301855
