...
UP_SAMPLE = {"EDA": [4, 24], "HR": [1, 2]}

PATH = "/hpc/group/sta440-f20/WESAD/WESAD/"

"""
For all of the CHEST metrics, SR was 700 Hz, so there are 700 * 60 samples/min and 350 samples represent 0.5 seconds
For EDA, with a final SR of 24Hz, there are 24 * 60 = 1440 samples/min and 12 samples represent 0.5 seconds
For TEMP, with a final SR of 4Hz, there is 4 * 60 = 240 samples/min, and 2 samples represents 0.5 seconds
For BVP, with a final SR of 64 Hz, there is 64 * 60 = 3840 samples/min, and 32 samples represents 0.5 seconds
For HR, with a final SR of 2Hz, there is 2 * 60 = 120 samples/min, and 1 sample represents 0.5 seconds
...
ROLL_APPLY = {"EMG_CHEST": [42000, 350, {"MEAN": np.mean, "STDDEV": np.std, "MEDIAN": np.median}],

              "ECG_CHEST": [42000, 350, {"MEAN": np.mean, "STDDEV": np.std}],

              "EDA_CHEST": [42000, 350, {"MEAN": np.mean, "STDDEV": np.std, "MAX": np.max}],

              "Temp_CHEST": [42000, 350, {"MEAN": np.mean, "STDDEV": np.std, "MEDIAN": np.median}],

              "Resp_CHEST": [42000, 350, {"MEAN": np.mean, "STDDEV": np.std}],

              "EDA_EMP4": [1440, 12, {"MEAN": np.mean, "STDDEV": np.std, "MAX": np.max}],

              "TEMP_EMP4": [240, 2, {"MEAN": np.mean, "STDDEV": np.std, "MEDIAN": np.median}],

              "BVP_EMP4": [3840, 32, {"MEAN": np.mean, "STDDEV": np.std}],

              "HR_EMP4": [120, 1, {"MEAN": np.mean, "STDDEV": np.std}]}
...
def roll_apply(high_res_data, func=np.mean, window=42000, shift=350, goal_length=None):
...
if os.path.exists("status_csv_2hz.txt"):
    os.remove("status_csv_2hz.txt")

status_file = open(r"status_csv_2hz.txt", "a")
...
merged_df_name = "merged_wesad_data_2Hz.csv"
