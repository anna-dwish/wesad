...
"""
For all of the CHEST metrics, SR was 700 Hz, so there are 700 * 120 samples per 2min and 21000 samples represent 30 sec
For EDA, with a final SR of 24Hz, there are 24 * 120 = 2880 samples per 2min and 720 samples represent 30 seconds
For TEMP, with a final SR of 4Hz, there is 4 * 120 = 480 samples per 2min, and 120 samples represents 30 seconds
For BVP, with a final SR of 64 Hz, there is 64 * 120 = 7680 samples per 2min, and 1920 samples represents 30 seconds
For HR, with a final SR of 1Hz, there is 1 * 120 = 120 samples per 2min, and 30 samples represents 30 seconds
...
ROLL_APPLY = {"EMG_CHEST": [84000, 21000, {"MEAN": np.mean, "STDDEV": np.std, "MEDIAN": np.median}],

              "ECG_CHEST": [84000, 21000, {"MEAN": np.mean, "STDDEV": np.std}],

              "EDA_CHEST": [84000, 21000, {"MEAN": np.mean, "STDDEV": np.std, "MAX": np.max}],

              "Temp_CHEST": [84000, 21000, {"MEAN": np.mean, "STDDEV": np.std, "MEDIAN": np.median}],

              "Resp_CHEST": [84000, 21000, {"MEAN": np.mean, "STDDEV": np.std}],

              "EDA_EMP4": [2880, 720, {"MEAN": np.mean, "STDDEV": np.std, "MAX": np.max}],

              "TEMP_EMP4": [480, 120, {"MEAN": np.mean, "STDDEV": np.std, "MEDIAN": np.median}],

              "BVP_EMP4": [7680, 1920, {"MEAN": np.mean, "STDDEV": np.std}],

              "HR_EMP4": [120, 30, {"MEAN": np.mean, "STDDEV": np.std}]}
...
def roll_apply(high_res_data, func=np.mean, window=84000, shift=21000, goal_length=None):
...
def apply_mutations(df, x, metric):
    ...
    for sub_title in mutations_dict:
        ...
        if len(df) != 0 and len(mutated_metric) < len(df):
            mutated_metric = list(mutated_metric)
...
if os.path.exists("status_csv_0_5hz.txt"):
    os.remove("status_csv_0_5hz.txt")

status_file = open(r"status_csv_0_5hz.txt", "a")
...
merged_df_name = "merged_wesad_data_0_5Hz.csv"
